...
const response = (statusCode, body) => ({ // create a response object to return to the client with the status code and body of the response (the body is a JSON object)
...
const createStream = async (event) => {  // create a new stream for a user (POST) - this is the function that is called by the client when they want to create a new stream (POST) 
    const requestBody = JSON.parse(event.body); // parse the request body       
    const params = { // create a params object to pass to our dynamoDB client
      ...
      ExpressionAttributeNames: { // name our attributes
        "#uid": "userId", // user id
      },
      
      ExpressionAttributeValues: { // value our attributes
        "uid": requestBody.userId, // user id
      },
    };

    const stream = {   // create a stream object to pass to our dynamoDB client to create a new stream log entry in our 
    ...
    if (!stream.userId) { // if the user id is not provided in the request body return a 400 error response with a message saying that the user id is required (userId is required)
      ...
      return dynamodb.scan(params, (error, data) => { //
        if (data.Items.length < 3) { // if the user is not currently watching more than 3 video streams, create a new stream log entry in our table for the user with the given user id (userId) and a unique stream id (streamId) and a timestamp (timestamp)
          dynamodb.put({   // create a new stream log entry in our table for the user with the given user id (userId) and a unique stream id (streamId) and a timestamp (timestamp)
            ...
            .then(() => {// create a new stream log entry in our table for the user with the given user id (userId) and a unique stream id (streamId) and a timestamp (timestamp)
              callback(null, response(201, stream)); //  callback(null, response(201, stream)) is a callback function that is called when the request is complete. It takes two arguments: an error and the response. The error argument is null if the request succeeded. The response argument is a JSON object with the following properties: statusCode, body. statusCode is the HTTP status code of the response. body is the body of the response.
            })
            .catch((error) => {  // create a new stream log entry in our table for the user with the given user id (userId) and a unique stream id (streamId) and a timestamp (timestamp)
...
const deleteStream = async (event) => {  // create a new stream for a user (POST) - this is the function that is called by the client when they want to create a new stream (POST)
    const requestBody = JSON.parse(event.body); // parse the request body
    const params = { // create a params object to pass to our dynamoDB client
        TableName: TABLE_NAME, // name of our table
        ProjectionExpression: "#uid", // only return the user id
    ...
    if (!stream.userId) { // if the user id is not provided in the request body return a 400 error response with a message saying that the user id is required (userId is required)
        callback(null, response(400, { error: "Request missing userId" })); // callback(null, response(400, { error: "Request missing userId" }))  is a callback function that is called when the request is complete. It takes two arguments: an error and the response. The error argument is null if the request succeeded. The response argument is a JSON object with the following properties: statusCode, body. statusCode is the HTTP status code of the response. body is the body of the response.
    } else { // if the user id is provided in the request body
        return dynamodb.scan(params, (error, data) => { //
            if (data.Items.length > 0) { // if the user is currently watching more than 0 video streams, delete the stream log entry in our table for the user with the given user id (userId) and a unique stream id (streamId) and a timestamp (timestamp)
                dynamodb.delete({ // delete the stream log entry in our table for the user with the given user id (userId) and a unique stream id (streamId) and a timestamp (timestamp)
                    TableName: TABLE_NAME, // name of our table
                    Key: { // create a key object to pass to our dynamoDB client to delete the stream log entry in our table for the user with the given user id (userId) and a unique stream id (streamId) and a timestamp (timestamp)
                    ...
                    .then(() => { // delete the stream log entry in our table for the user with the given user id (userId) and a unique stream id (streamId) and a timestamp (timestamp)
                        callback(null, response(204, {})); // callback(null, response(204, {})) is a callback function that is called when the request is complete. It takes two arguments: an error and the response. The error argument is null if the request succeeded. The response argument is a JSON object with the following properties: statusCode, body. statusCode is the HTTP status code of the response. body is the body of the response.
                    })
                    .catch((error) => { // delete the stream log entry in our table for the user with the given user id (userId) and a unique stream id (streamId) and a timestamp (timestamp)
                        callback(null, response(error.statusCode, { message: error })); // callback(null, response(error.statusCode, { message: error })) is a callback function that is called when the request is complete. It takes two arguments: an error and the response. The error argument is null if the request succeeded. The response argument is a JSON object with the following properties: statusCode, body. statusCode is the HTTP status code of the response. body is the body of the response.
                    });
            } else { // if the user is currently watching 0 video streams, return a 400 error response with a message saying that the user is currently watching 0 video streams (userId is required)
                callback(
                    null,
                    response(400, {
